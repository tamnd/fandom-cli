...
describes what users get. Pick a format with `-o`, or let fandom choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
fandom <command> -o table   # aligned columns for reading
fandom <command> -o jsonl   # one JSON object per line, for piping
fandom <command> -o json    # a single JSON array
fandom <command> -o csv     # spreadsheet friendly
fandom <command> -o tsv     # tab-separated
fandom <command> -o url     # just the URL column
fandom <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
fandom <command> --fields id,title,url
```
...
```bash
fandom <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
fandom <command>            # a table, because this is a terminal
fandom <command> | wc -l    # JSONL, because this is a pipe
```
